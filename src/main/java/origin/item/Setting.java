...
public class Setting implements ActivityItem {
    ...
    public void start(String line) throws Exception {
        ...
        //web
        if (webPageVariable.check(line)) webPageVariable.start(line);
        else if (portVariable.check(line)) portVariable.start(line);
        else if (startServer.check(line)) startServer.start(line);
    }
    ...
    public void varClear() {
        ...
        //web
        Temporary.portMap.clear();
        Temporary.pathMap.clear();
    }
    ...
    protected String  checkValue(@NotNull String word) {
        ...
        else if (SM.containsKey(word)) return SM.getOrDefault(word, "");
        else return Temporary.portMap.get(word);
    }
